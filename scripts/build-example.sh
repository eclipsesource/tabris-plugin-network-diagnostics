#!/usr/bin/env bash
# Builds one of the example apps against the plugin checked out in this
# repository. Called through example/build.sh and example_typescript/build.sh.
#
#   build-example.sh <example-dir>              iOS Simulator, debug, developer console on  -> APP_PATH=
#   build-example.sh <example-dir> --app-store  device, release, developer console off      -> IPA_PATH=
#
...
# step.
#
# The TypeScript example also lists the plugin as a devDependency ("file:..")
# so that tsc finds its type declarations before Cordova has installed anything;
# that reference is rewritten to the same staging directory.
set -euo pipefail

USAGE="usage: build-example.sh <example-dir> [--app-store]"
if [[ -z "${1:-}" || ! -d "$1" ]]; then
    echo "$USAGE" >&2
    exit 2
fi
EXAMPLE_APP_DIR="$(cd "$1" && pwd)"
EXAMPLE_NAME="$(basename "$EXAMPLE_APP_DIR")"
...
cp -R "$EXAMPLE_APP_DIR" "$WORK_DIR/$EXAMPLE_NAME"
rm -rf "$WORK_DIR/$EXAMPLE_NAME/build" "$WORK_DIR/$EXAMPLE_NAME/dist" "$WORK_DIR/$EXAMPLE_NAME/node_modules"
cd "$WORK_DIR/$EXAMPLE_NAME"
...
if grep -q '"file:\.\."' package.json; then
    sed -i '' "s#\"file:\\.\\.\"#\"file:$PLUGIN_STAGE\"#" package.json
    test "$(grep -c "\"file:$PLUGIN_STAGE\"" package.json)" -eq 1
...
if [[ "$APP_STORE_BUILD" == false ]]; then
    ...
    APP_PATH="$WORK_DIR/$EXAMPLE_NAME/build/cordova/platforms/ios/build/emulator/$APP_NAME.app"
    test -d "$APP_PATH"
    printf '%s\n' "$APP_PATH" > "/tmp/claude/netdiag-$EXAMPLE_NAME-app-path.txt"
...
ARCHIVE="$WORK_DIR/$EXAMPLE_NAME/build/cordova/platforms/ios/$APP_NAME.xcarchive"
test -d "$ARCHIVE" || {
...
printf '%s\n' "$IPA_PATH" > "/tmp/claude/netdiag-$EXAMPLE_NAME-ipa-path.txt"
